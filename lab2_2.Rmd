...
```{r}
days_of_the_week[4] #square pull outs specific elements
```
...
```{r}
names(hbirds) #names function gives the column names
```

What are the dimensions of the `hbirds` data frame? The `dim()` and `str()` commands provide this information.   
```{r}
dim(hbirds) #dim function gives the dimensions and this is 3x3
```

```{r}
str(hbirds) #str function gives the structure of the data frame
```
...
The first row.  
```{r}
hbirds[1,] #[num,] pulls out the row
```

The third column.  
```{r}
hbirds[ ,3] #[,num] pulls out the column
```
