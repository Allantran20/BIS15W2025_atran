...
```{r}
filter(fish, lakeid=="LJ") #the difference between filter and select function is that filter will give you the entire row of data, while select will only give you the columns you want. 
```
...
```{r}
filter(fish, lakeid == "AL" & length > 350) # & means both conditions must be met
```

Notice that the `|` operator generates a different result. Why?
```{r}
filter(fish, lakeid == "AL" | length > 350) # | means either condition can be met. which is the same as "or"
```
...
```{r}
filter(fish, length > 400, (scalelength > 11 | radii_length_mm > 8)) # length greater than 400 but also scalelength larger than 1 OR radii length larger than 8. scalelength and radii are being put inside the length function. 
```
...
```{r}
q3 <- filter(mammals, family == "Felidae" & gestation < 3)
q3_new <- select(q3, "genus", "family", "gestation")
q3_final <- filter(q3_new, gestation != -999)
q3_final
```
