...
```{r}
head(fish) #prints the first 6 rows
```

`tail()` prinst the last n rows of the data frame.
```{r}
tail(fish) #prints the last 6 rows
```
...
```{r}
select(fish, lakeid, length) #selects the lakeid and length columns. in this case it is lakeid and length instead of having all 6 variables
```
...
```{r}
filter(fish, length<=100) #filter out data in the rows where length is less than or equal to 100
```
...
#str(mammals)
glimpse(mammals)
#the difference between str and glimpse is that str shows the structure of the data frame and its individual columns, while glimpse shows the structure of the data frame and its individual columns in a more concise way
```
