...
``` r
# The mean mass of the animals is very large. This is likely because the data are in grams and not kilograms. Also there are some questionable negative within the data that suggest a revisit at those datas (for example in the mass as well as hra)
```
...
``` r
biggestmass1 = max(filter(homerange, taxon == "mammals")$mean.hra.m2)
biggestmass1
```
...
``` r
filter(homerange, mean.hra.m2 == biggestmass1)
```
...
``` r
# The animal with the largest home range is the reindeer. This is likely because they are migratory and have a large range of movement.
```
