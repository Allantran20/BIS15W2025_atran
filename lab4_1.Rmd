...
```{r}
select(fish, "lakeid", "scalelength") #this will return a new dataframe with only the columns lakeid and scalelength
```

Remember that you can save the output to a new dataframe.
```{r}
fish_subset <- select(fish, "lakeid", "scalelength") #this will return a new dataframe with only the columns lakeid and scalelength
```

To add a range of columns use `start_col:end_col`.
```{r}
select(fish, fish_id:length) #this will return a new dataframe with all columns from fish_id to length
```

The - operator is useful in select. It allows us to select everything except the specified variables.
```{r}
select(fish, -"fish_id", -"annnumber", -"length", -"radii_length_mm") #this will return a new dataframe with all columns except fish_id, annnumber, length, and radii_length_mm
```
...
```{r}
select(fish, contains("length")) #this will return a new dataframe with all columns that contain the word "length"
```

When columns are sequentially named, `starts_with()` makes selecting columns easier.
```{r}
select(fish, starts_with("radii")) #this will return a new dataframe with all columns that start with "radii"
```
...
anyNA(mammals)
mean(mammals$max.life, na.rm = TRUE) #na.rm removes NAs
```
...
```{r}
mammals_new <- rename(mammals, genus = "Genus") #only run rename once
```
...
```{r}
select_all(mammals, tolower) #this will return a new dataframe with all column names in lowercase
```

Another option is to use the janitor package. This package has a function called `clean_names()` that will make all column names lowercase and replace spaces with underscores.
```{r}
library(janitor)
names(mammals)
```
