...
```{r}
write.csv(hsprings, "hsprings_data.csv", row.names = FALSE) #row.names = FALSE prevents R from adding a column of row numbers
```
...
```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist) #change the class of the scientist column to factor
```
...
```{r}
levels(hot_springs$spring) #levels doesn't work on numeric columns and character. ONLY factors.
```
