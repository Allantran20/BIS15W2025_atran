...
else <- 12 #it is a function so can't be used
```
...
#this as.integer is a function to change the class of an object
```
...
```{r}
#The mean is NA because there is missing data in the vector.
```


4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm = TRUE) #na.rm stands for remove NAs
```
